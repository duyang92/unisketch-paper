...
class DetectorSS {
   ...
   public:
    DetectorSS();
    DetectorSS(int depth, int width, int lgn, int b, int c, int memory, int round = 0);
    ...
    std::unordered_map<uint32_t, double> top_k();
...
std::unordered_map<uint32_t, double> DetectorSS::top_k() {
    std::unordered_map<uint32_t, double> ss_result;
    uint32_t** skey = GetKey();
    for (int i = 0; i < ss_.depth; i++) {
        for (int j = 0; j < ss_.width; j++) {
            uint32_t it = skey[i][j];
            uint32_t degree = PointQueryMerge(it);
            ss_result[it] = degree;
        }
    }
    return ss_result;
...
void DetectorSS::Mergebmp(unsigned char* bmp1, unsigned char* bmp2, int start, int len) {
    int curlen = 0;
    while (curlen < len) {
        unsigned char temp = 0;
        int n = start + curlen;
